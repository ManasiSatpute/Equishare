...
export const getEquipmentById = (id) => {
  return new Promise((resolve, reject) => {
    db.query("SELECT * FROM equipment WHERE id = ?", [id], (err, results) => {
      if (err) return reject(err);
      resolve(results[0]); // ✅ Always return a single item
    });
  });
};
...
export const createEquipment = (equipment) => {
  const query = `
    ...
    (owner_id, name, category, description, price, location, image_url, engine_power, fuel_type, year, equipment_condition)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
